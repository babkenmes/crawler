...
def Animation1(driver):
    ...
    time.sleep(abs((gauss(1, 1) * 500 + 7000))/1000)
    menu_list = ["NEWS","REVIEWS","EDUCATION","LAB REPORT","CONTACT"]
    menu_item_1 = random.choice(menu_list)
    print(menu_item_1)
    time.sleep(abs((gauss(1, 1) * 500 + 4000))/1000)
    try:
        driver.find_element_by_xpath('//*[@id="page-header"]/div[2]/div/div[1]/nav/a').click()
    except:
        pass    
    time.sleep(abs((gauss(1, 1) * 500 + 9000))/1000)
    menu_list = driver.find_element_by_xpath(f"//span[contains(text(),'{menu_item_1}')]").click()
    time.sleep(abs((gauss(1, 1) * 500 + 8000))/1000)
    driver.execute_script("window.scrollTo(0, 2000)")
    time.sleep(abs((gauss(1, 1) * 500 + 8000))/1000)
    driver.execute_script("window.scrollTo(0, 20)")
    ...
    try:
        ...
        pass    
    time.sleep(abs((gauss(1, 1) * 500 + 2000))/1000)
    menu_list = driver.find_element_by_xpath(f"//span[contains(text(),'{menu_item_2}')]").click()
    time.sleep(abs((gauss(1, 1) * 500 + 2000))/1000)
    driver.execute_script("window.scrollTo(0, 2000)")
    time.sleep(abs((gauss(1, 1) * 500 + 2000))/1000)
    driver.execute_script("window.scrollTo(0, 20)")
