# -*- coding: utf-8 -*-
from webmen import getAllDevices, getDevice, getWeb, getKey, keyDone, keyError, keyStarted, keyStopped, recapDetected
    ...
    while True:
        try:
            try:
                ...
                if key_data["tld"] == "direct":
                    goDirect(driver, key_data[KEY_FIELD])
                ...
                keyError(
                    key_data["_id"], f"website is not clickable web:{WEB[DOMAIN_FIELD]}, exception: {e}")
            except AnimationError as e:
                keyError(key_data["_id"],
                        f"Animation exception has accured, exception: {e}")
            except NoDeviceError as e:
                keyError(
                    key_data["_id"], f"No device was found group:{key_data[DEVICE_GROUP_FIELD]}")
            except WebNotFound as e:
                keyError(
                    key_data["_id"], f"Web could not be found in {MAX_PAGES} pages, web: {WEB[DOMAIN_FIELD]}")
            except NextButtonNotFound as e:
                keyError(
                    key_data["_id"], f"Button for next page not be found, device:{device[DEVICE_NAME_FIELD]}, isMobile:{device[MOBILE_FIELD]}, button text: {MORE_RESULTS_TEXT}")
            except NextButtonCouldNotBeClicked as e:
                keyError(
                    key_data["_id"], f"Next page button could not be clicked, device:{device[DEVICE_NAME_FIELD]}, isMobile:{device[MOBILE_FIELD]}, button text: {MORE_RESULTS_TEXT}")
            except Exception as e:
                keyError(key_data["_id"],
...
def getConnections(docker_config_file):
    ...
    for conect in compose_configs["conects"]: 
        regs = (conect["regions"] * compose_configs["NODE_MAX_SESSION"]) * [conect["region_tag"] ]
        connections = [*connections, *regs]
